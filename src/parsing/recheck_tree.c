...
/*   Updated: 2024/02/07 12:10:52 by lkary-po         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	replace_file(t_s_token *node)
{
	while (node)
	{
		if (node && node->token_type == TK_OP && (node->data.op.type == REDIR_IN || node->data.op.type == REDIR_OUT || node->data.op.type == REDIR_APPEND))
